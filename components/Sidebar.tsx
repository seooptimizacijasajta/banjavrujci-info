  ...
  return (
    <aside className="space-y-6">
      <div className="bg-white rounded-xl shadow-sm p-4">
        <h3 className="font-bold mb-3">Izdvojeni smeštaj</h3>
        <div className="space-y-3">
          {shuffled.map((l: any) => (
            <Link key={l.slug} href={`/smestaj/${l.slug}`} className="flex gap-3 items-center group">
        ...
        <Link href="/smestaj" className="block mt-3 text-sm text-brand font-semibold">Sav smeštaj →</Link>
      </div>
      <Link href="/kontakt" className="block bg-brand text-white rounded-xl p-4 text-center font-semibold hover:bg-brand-dark">Kontakt</Link>
