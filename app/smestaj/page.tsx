...
export default async function Smestaj() {
  ...
  const markers = (listings||[]).filter((l:any)=>l.latitude&&l.longitude).map((l:any)=>({lat:l.latitude,lng:l.longitude,title:l.title,href:`/smestaj/${l.slug}`}));
  return (
    <div className="grid lg:grid-cols-[1fr_320px] gap-8 py-6">
      <div className="min-w-0">
        <h1 className="text-3xl font-bold mb-2">Smeštaj u Banji Vrujci</h1>
        <p className="text-slate-600 mb-6">Ukupno {listings?.length || 0} objekata — apartmani, vile, sobe, hoteli, bungalovi i privatni smeštaj.</p>
        {markers.length>0 && (<div className="mb-8"><h2 className="text-xl font-bold mb-3">Smeštaj na mapi</h2><ListingMap markers={markers} zoom={14} height="420px" /></div>)}
        {cats.length===0 && <p className="text-slate-600">Trenutno nema objavljenih objekata.</p>}
        <div className="space-y-10">
          {cats.map((c)=>(
            <section key={c}>
              <h2 className="text-2xl font-bold mb-4">{CAT_TITLES[c] || c} <span className="text-slate-400 text-lg">({byCat[c].length})</span></h2>
