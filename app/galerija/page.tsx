  ...
  return (
    <div className="grid lg:grid-cols-[1fr_320px] gap-8 py-6">
      <div className="min-w-0 space-y-6">
        <nav className="text-sm text-slate-500"><Link href="/" className="hover:text-brand">Početna</Link> / <span className="text-brand">Galerija</span></nav>
        <h1 className="text-3xl font-bold">Galerija Banje Vrujci</h1>
        <p className="text-slate-600">Fotografije po temama — bazeni, priroda, okolina, manastiri i znamenitosti.</p>
        ...
        {rest.length > 0 && (
          <div className="flex flex-wrap gap-2 pt-2">
            {rest.map((k: any) => <Link key={k.slug} href={`/${k.slug}`} className="text-sm bg-brand/10 text-brand rounded-full px-3 py-1 hover:bg-brand hover:text-white">{k.title.replace(/^Banja Vrujci\s*/i,"").replace(/\s*[-–,].*$/,"")}</Link>)}
          </div>
        )}
      </div>
      <Sidebar />
